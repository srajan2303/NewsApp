...
const News = (props) => {
    ...
    useEffect(() => {
        document.title = `${capitalizeFirstLetter(props.category)} - DailyDive`;
        updateNews();
    }, [])
    ...
    return (
        <>
            <h1 className="text-center" style={{ margin: '35px 0', marginTop: '90px' }}>DailyDive - Top {capitalizeFirstLetter(props.category)} Headlines</h1>
            {loading && <Spin />}
            <InfiniteScroll
